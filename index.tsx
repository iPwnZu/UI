...
function App() {
          ...
          try {
              ...
              const response = await ai.models.generateContent({
                  ...
                  contents: { 
                      role: 'user', 
                      parts: [{ 
                          text: 'Generate 20 creative, short, diverse UI component prompts (e.g. "bioluminescent task list"). Return ONLY a raw JSON array of strings in Czech language. IP SAFEGUARD: Avoid referencing specific famous artists, movies, or brands.' 
                      }] 
                  }
              });
  ...
  const handleGenerateVariations = useCallback(async () => {
    ...
    setDrawerState({ isOpen: true, mode: 'variations', title: 'Varianty', data: currentArtifact.id });
...
**YOUR TASK:**
For EACH variation:
- Invent a unique design persona name based on a NEW physical metaphor (You may use Czech or English for the name).
...
\`{ "name": "Persona Name", "html": "..." }\`
  ...
  const handleDownload = () => {
      ...
      if (currentSession && focusedArtifactIndex !== null) {
          ...
          a.href = url;
          a.download = `pwnz-ui-${artifact.id}.html`;
...
**GOAL:**
Return ONLY a raw JSON array of 3 *NEW*, creative names for these directions in Czech language (e.g. ["Hmatatelný Risograf", "Kinetická Silueta", "Pigmentová Mřížka"]).
        `.trim();
          ...
          `.trim();
                ...
                setSessions(prev => prev.map(sess => 
                    sess.id === sessionId ? {
                        ...sess,
                        artifacts: sess.artifacts.map(art => 
                            art.id === artifact.id ? { ...art, html: `<div style="color: #ff6b6b; padding: 20px;">Chyba: ${e.message}</div>`, status: 'error' } : art
                        )
                    } : sess
                ));
            }
